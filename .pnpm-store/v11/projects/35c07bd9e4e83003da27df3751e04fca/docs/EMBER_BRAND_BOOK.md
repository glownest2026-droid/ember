...
Use this document for any new page, component, or copy surface. If ThemeProvider / admin theme injects Inter or Source Serif 4, **do not** let those win on public Discover or marketing surfaces â€” force Manrope + the tokens below.

Related: Conor voice and copy bar â†’ `web/docs/CONSCIENTIOUS_CONOR.md`
Code tokens â†’ `web/src/lib/discover/figmaTokens.ts`, `web/src/lib/discover/navStyles.ts`, `web/src/lib/discover/manrope.ts`, `web/src/lib/marketing/layout.ts`, `.homepage-discover-brand` in `web/src/app/globals.css`

---

## 1. Brand feel

Warm, calm, premium UK parenting â€” not clinical, not toy-catalogue shouty, not generic purple SaaS.
...
**Manrope** only â€” weights **400 / 500 / 600 / 700**.
...
### Discover (in-app) scale â€” product source of truth

From `DiscoverFigmaChildHero`:

| Role | Size | Weight | Colour |
|------|------|--------|--------|
| Page title (h1) | 32px â†’ 44px md | 700 | `#253044` |
| Body / hero sub | 16px â†’ 17px md | 400 | `#66717D` |
...
|-------|------|---------------|--------|
| `.home-display` | Hero title | 48 â†’ 60 â†’ 80 â†’ 88px | 600 |
| `.home-lead` / `.home-section-lead` / `.home-body` | **One body size pagewide** | 20 â†’ 22 â†’ 24px | 400 |
| `.home-section-title` | Section h2 | 32 â†’ 36 â†’ 44px | 600 |
| `.home-pullquote` | Statement line | 28 â†’ 32 â†’ 36px | 500 |
| `.home-card-title` | Card titles | 22 â†’ 24px | 600 |
| `.home-cta` / `.home-link` | Buttons & text CTAs | 18 â†’ 20px | 500 |
...
| Signed-out nav height | ~96â€“112px (`h-24` / `h-28`) | Fits larger robin mark |
| Homepage hero | `min-h` â‰ˆ viewport minus nav | Age slider sits below the fold |
...
- **Robin mark:** ~80Ã—80 mobile, ~88Ã—88 desktop (legible brand presence).
- **Wordmark â€œEmberâ€:** `text-xl` bold ink â€” **not** doubled with the icon; must sit with nav link scale.
- Asset: `brand-assets/logos/Ember_Logo_Robin1.png` (Supabase public).

### Primary CTA (filled)

```
bg #FF5C34 Â· text #FFFFFF Â· rounded-xl Â· font-medium/semibold
...
Ink or accent text, medium weight, optional arrow â€” no filled background.

### Cards / media

- Radius ~20â€“24px (`rounded-[20px]` / `home-media` 1.5rem).
...
- Prefer ~0.5â€“0.8s fade/slide with `motion/react`.
...
Banned in public copy: magic, win, unlock, optimise, essential, must-have, â€œresearch showsâ€, vague leap language.

**Parent-facing sentence craft (canonical):** `web/docs/brand/WRITING_GUIDELINES.md`  
**Persona five tests:** `web/docs/CONSCIENTIOUS_CONOR.md`  
**Brand folder map:** `web/docs/brand/README.md`

“Warm, calm, premium” here describes *visual feel*. Do not paste “calm” into product blurbs — Writing Guidelines ban it as product praise.
...
1. Manrope only â€” no Inter / Source Serif leak on the surface.
2. Body `#66717D`, headings `#253044`, accent `#FF5C34`.
3. Orange button â†’ white text.
...
| Homepage type scale | `web/src/app/globals.css` â†’ `.homepage-discover-brand` |
