...
function resolveVisibleCount(doc, fallback = 5) {
  const raw = String(doc.ownership_class || doc.research_depth || '')
    .trim()
    .toLowerCase();
  if (raw === 'multiple' || raw === 'collection' || raw === 'books') return 10;
  if (raw === 'single' || raw === 'standard') return 5;
  const topLen = asArray(doc.top_picks).length;
  if (topLen === 10) return 10;
  if (topLen === 5) return 5;
  return fallback;
}

function normalizeResearch(filePath, forcedAgeBand, visibleCountFallback = 5) {
  const raw = fs.readFileSync(filePath, 'utf8');
  const doc = JSON.parse(raw);
  const ageBandId = forcedAgeBand || doc.age_band || doc.age_band_id;
  const visibleCount = resolveVisibleCount(doc, visibleCountFallback);
  ...
  if (visibleCount >= 10 && longlist.length < 25) {
    warnings.push(`Multiple-ownership expected 25 longlist entries, got ${longlist.length}`);
  }
  ...
  const backupCap = visibleCount >= 10 ? 25 : 15;
  const backups = longlist
    .filter((item) => {
      ...
      return Number.isFinite(rank) && rank > visibleCount && rank <= backupCap && !included;
    })
    .sort((a, b) => Number(a.longlist_rank) - Number(b.longlist_rank));

  const expectedBackupFloor = Math.max(0, Math.min(5, backupCap - visibleCount));
...
function buildMigration(bundle, timestamp) {
  ...
  const expectedVisible = categories.reduce((n, c) => n + c.top_picks.length, 0);
...
function main() {
  ...
  const mixedVisible = [...new Set(categories.map((c) => c.visible_count))];
  const bundle = {
    schema_version: 'stage3_ingestion_bundle_v1',
    generated_at: new Date().toISOString(),
    age_band_id: ageBands[0],
    visible_count: mixedVisible.length === 1 ? mixedVisible[0] : 'mixed',
    visible_counts: mixedVisible,
    locked_from_rank: 2,
    founder_paid_proxy_email: 'timwd23@gmail.com',
    categories,
    acceptance_checks: {
      top_5_per_category: categories.every((c) => c.top_picks.length === c.visible_count),
      visible_count_per_category: categories.every((c) => c.top_picks.length === c.visible_count),
      backups_after_visible_per_category: categories.every((c) => {
        const floor = Math.max(0, Math.min(5, (c.visible_count >= 10 ? 25 : 15) - c.visible_count));
        return c.backup_picks.length >= floor;
      }),
