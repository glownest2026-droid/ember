...
function curatedLabel(firstName: string | null, gender: string | null): string {
...
export function DiscoverFigmaChildHero({
  ...
  const name = childFirstName?.trim() || null;
  const curated = curatedLabel(childFirstName, childGender ?? null);
  const headline = name ? `What ${name} is learning right now` : 'What your child is learning right now';
  const sub =
    name ?
      `At ${monthAge} months, ${name} is building skills that match this stage. Here's what matters most—and how to support ${name}.`
    : `At ${monthAge} months, your child is building skills that match this stage. Here's what matters most—and how to support them.`;
