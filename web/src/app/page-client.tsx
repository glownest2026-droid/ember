...
export default function HomePageClient({ isSignedIn }: { isSignedIn: boolean }) {
  return (
    <>
      <Hero showWaitlist={!isSignedIn} />
      <FeaturedStrip />
      <HowItWorks />
      <Demo />
      <FeatureSection />
...
function Hero({ showWaitlist }: { showWaitlist: boolean }) {
    ...
    >
      <div className="mx-auto max-w-6xl px-6 pt-24 pb-16 grid md:grid-cols-2 gap-10 items-center">
        <div>
          ...
          {/* Waitlist form - only show if not signed in */}
          {showWaitlist && (
            <form
              id="waitlist"
              onSubmit={onSubmit}
