-- Split shared ent_need_early_play junction rows into per-cluster needs so
-- v2 purchase-depth cards render only on their owning Stage 1 cluster.

INSERT INTO public.pl_development_needs (name, slug, plain_english_description, why_it_matters)
VALUES
  ('Early play — listening', 'ent_need_1_3_listen_play_depth', '', ''),
  ('Early play — watching', 'ent_need_1_3_watching_play_depth', '', ''),
  ('Early play — tummy', 'ent_need_1_3_tummy_play_depth', '', ''),
  ('Early play — wriggling', 'ent_need_1_3_kicks_play_depth', '', ''),
  ('Early play — hands', 'ent_need_1_3_grasps_play_depth', '', '')
ON CONFLICT (slug) DO NOTHING;

UPDATE public.pl_age_band_development_need_category_types j
SET development_need_id = dn.id, updated_at = now()
FROM (
  VALUES
    ('cat_first_play_guide_cards', 'ent_need_1_3_listen_play_depth'),
    ('cat_gentle_musical_touch_toy', 'ent_need_1_3_listen_play_depth'),
    ('cat_rolling_bell_sound_ball', 'ent_need_1_3_watching_play_depth'),
    ('cat_tummy_time_support_prop', 'ent_need_1_3_tummy_play_depth'),
    ('cat_crinkle_floor_book', 'ent_need_1_3_tummy_play_depth'),
    ('cat_kick_and_look_play_gym', 'ent_need_1_3_kicks_play_depth'),
    ('cat_washable_floor_play_mat', 'ent_need_1_3_kicks_play_depth'),
    ('cat_soft_hanging_toys', 'ent_need_1_3_kicks_play_depth'),
    ('cat_twist_stroke_tactile_toy', 'ent_need_1_3_grasps_play_depth'),
    ('cat_teething_cloth', 'ent_need_1_3_grasps_play_depth')
) AS v(category_slug, need_slug)
JOIN public.pl_category_types ct ON ct.slug = v.category_slug
JOIN public.pl_development_needs dn ON dn.slug = v.need_slug
WHERE j.age_band_id = '1-3m'
  AND j.category_type_id = ct.id
  AND j.is_active = true
  AND EXISTS (
    SELECT 1 FROM public.pl_development_needs old_dn
    WHERE old_dn.id = j.development_need_id
      AND old_dn.slug = 'ent_need_early_play'
  );
