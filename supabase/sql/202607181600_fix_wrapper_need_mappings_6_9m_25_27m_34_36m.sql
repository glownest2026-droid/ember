-- Root-cause fix for wrappers that resolved to zero Stage 2 categories.
--
-- v_gateway_age_band_wrapper_needs_public is the age-band-specific wrapper → need
-- mapping. Three bands had gaps, previously patched in application code
-- (a hardcoded 34-36m override and a fuzzy keyword matcher added in PR #264).
-- This migration moves the truth into the mapping view where it belongs:
--
--   6-9m   : cluster_first_foods / cluster_mouth_sensory — the legacy global link
--            points at the 4-6m needs; the 6-9m categories live under needs whose
--            slugs match the wrapper slugs.
--   25-27m : the Conor/Thea v2 reimport (20260703090000) rehomed categories under
--            new need slugs but the mapping view was never updated.
--            ent_cluster_busy_hands_puzzles spans two needs (source workbook rows
--            for need_busy_hands landed under need_two_handed_control).
--   34-36m : Spine 3.0 import uses need_language_story / need_social_emotional while
--            the legacy wrapper link points at old empty 28-30m needs.
--
-- All other rows are unchanged from the live view definition.
...
WITH spine_v2_mapping (age_band_id, wrapper_slug, need_slug, need_rank) AS (
  VALUES
    ...
    ('6-9m', 'cluster_first_foods', 'cluster_first_foods', 1),
    ('6-9m', 'cluster_mouth_sensory', 'cluster_mouth_sensory', 1),
    ...
    ('25-27m', 'ent_cluster_big_movement', 'need_gross_motor_outdoor', 1),
    ('25-27m', 'ent_cluster_busy_hands_puzzles', 'need_fine_motor_problem_solving', 1),
    ('25-27m', 'ent_cluster_busy_hands_puzzles', 'need_two_handed_control', 2),
    ('25-27m', 'ent_cluster_feelings_turn_taking', 'need_social_emotional_turn_taking', 1),
    ('25-27m', 'ent_cluster_home_rhythm_safety', 'need_parent_routines_safety', 1),
    ('25-27m', 'ent_cluster_independence_practice', 'need_independence_self_care', 1),
    ('25-27m', 'ent_cluster_potty_toilet_practice', 'need_potty_toilet_awareness', 1),
    ('25-27m', 'ent_cluster_pretend_helper_play', 'need_pretend_imagination', 1),
    ('25-27m', 'ent_cluster_talk_stories', 'need_language_sentences', 1),
    ...
    ('34-36m', 'ent_cluster_talk_stories_questions', 'need_language_story', 1),
    ('34-36m', 'ent_cluster_feelings_turn_taking', 'need_social_emotional', 1)
)
SELECT
  m.age_band_id,
