-- Age-band-specific wrapper → development_need mapping for spine v2 bands.
-- pl_ux_wrapper_needs is global (one need per wrapper slug), which breaks category
-- lookup when the same wrapper slug maps to different needs per age band
-- (e.g. ent_cluster_words → ent_need_books in 9-12m vs ent_need_13_15_words_pointing in 13-15m).
