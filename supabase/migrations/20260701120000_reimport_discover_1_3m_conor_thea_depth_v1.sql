...
BEGIN
  ...
  RAISE NOTICE 'Discover projection rows loaded: % (expected 45)', v_rows_loaded;
  RAISE NOTICE '1-3m rows: % (expected 45), clusters: % (expected 10)', v_rows_1_3m, v_clusters_1_3m;
  RAISE NOTICE 'Active Stage 3 mappings (expected 0): %', v_stage3_active;

  IF v_rows_loaded <> 45 THEN
    RAISE EXCEPTION 'Row count validation failed';
  END IF;
  IF v_rows_1_3m <> 45 THEN
