...
- **Discover personalization (server):** `getDiscoverServerPersonalization(childParam)` returns `displayLabel`, `gender`, `monthsOld` (from birthdate); **`serverPersonalization`** + **`key={discover-${month}-${child}}`** for first-paint hero.
