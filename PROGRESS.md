...
- **Discover personalization (server):** `getDiscoverServerPersonalization(childParam)` in RSC loads name/gender with session cookie; passed as **`serverPersonalization`** + **`key={discover-${month}-${child}}`** so hero shows “Ben” on first paint (fixes client `useSearchParams`/user timing clearing profile).
